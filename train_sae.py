...
parser.add_argument('--activation_dir', type=str, default='activations/')
...
def load_model(local_rank):
    model = SAE(input_dim=4096, hidden_dim=4096 * 4).to(torch.bfloat16)
...
def validate(model, validation_loader, local_rank, writer, epoch_num):
    ...
    if local_rank == 0:
        ...
        writer.add_scalar('Loss/val_total', avg_val_mse + model.module.l1_lambda * avg_val_l1, epoch_num)
    
    return avg_val_mse, avg_val_l1

def train(model, local_rank):
    train_loader, validation_loader = get_data_loaders(args.activation_dir, args.layer_num, batch_size=64)
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
    
    # Setup TensorBoard only on main process
    if local_rank == 0:
        current_time = datetime.now().strftime('%b%d_%H-%M-%S')
        writer = SummaryWriter(f'{args.output_dir}/sae_{current_time}')
    ...
    for epoch_num in range(args.num_epochs):
        ...
        for step, tqdm_batch in enumerate(tqdm(train_loader, desc=f'Train Epoch {epoch_num}', disable=local_rank != 0)):
            ...
            mse_loss = torch.nn.functional.mse_loss(reconstruction, batch)
            l1_loss = torch.nn.functional.l1_loss(sparse_representation, torch.zeros_like(sparse_representation))
            loss = mse_loss + model.module.l1_lambda * l1_loss
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
            ...
            if (step + 1) % 10 == 0:
                ...
                if local_rank == 0:
                    ...
                    writer.add_scalar('Loss/step_total', avg_step_mse + model.module.l1_lambda * avg_step_l1, epoch_num * len(train_loader) + step)
        ...
        if local_rank == 0:
            ...
            writer.add_scalar('Loss/train_total', avg_train_mse + model.module.l1_lambda * avg_train_l1, epoch_num)
